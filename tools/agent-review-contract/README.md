...
The Codex automation initializes a native durable Goal for the currently executable directed-review frontier. It has no candidate, full-review, refresh-cycle, or elapsed-time work budget: ordinary queue volume is processed to the fixed point. A configurable repeated-no-progress guard prevents livelock, and the zero-spend default remains an authority boundary. Finishing that bounded Goal is not the same as declaring the open-ended global queue complete.
...
The automation derives the current reviewer identity and current repository state at runtime. Governing instructions come from the trusted integration-base revision; candidate instruction changes remain review inputs until integrated. Each deciding oracle runs from a fresh immutable exact-pin snapshot with read-only inputs, no provider credentials or network, and separate ephemeral outputs; any input mutation invalidates its result. Authenticated provider reads and submission use a separate trusted step. Private-security work is admitted only when the reviewer already has authorized access and the task explicitly includes that venue. General task results and notifications remain disclosure-safe; detailed restricted records are emitted only to a verified authorized restricted destination.
...
- a repeated-no-progress livelock guard and incremental-spend limit, with no review-count budget.
...
Every disposition records a canonical assessment-input fingerprint covering reviewer and authority, addressed condition and scope, assessed stage, governing revisions, candidate and integration base, effective effect, and required-oracle/check and clearing-evidence state. Trigger, owner, publication, coordination-write, and readback results are excluded because the disposition itself can change them; an admission fingerprint adds the current trigger and owner for the immediate pre-submit drift check. A submitted disposition remains reusable only while its assessment fingerprint is current and its effects are reconciled separately. Scheduled read-only or draft execution keys durable runtime-local hold state by the assessment fingerprint plus current admission and hold state. Reliable read-only execution may update only bounded runtime-local Goal, lock, cursor, and hold state, never provider or project state. Missing or unknown provider writes enter reconciliation-only handling and are never retried blindly; an unauthorized missing write is held for the required authorization or human update rather than treated as executable.
