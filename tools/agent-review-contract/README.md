...
The automation derives the current reviewer identity and current repository state at runtime. Governing instructions come from the trusted integration-base revision; candidate instruction changes remain review inputs until integrated. Candidate-controlled commands run without provider credentials in an isolated network-denied workspace, while authenticated provider reads and submission use a separate trusted step. Private-security work is admitted only when the reviewer already has authorized access and the task explicitly includes that venue. General task results and notifications remain disclosure-safe; detailed restricted records are emitted only to a verified authorized restricted destination.
