...
Scheduled read-only or draft execution requires durable runtime-local hold state keyed by the contract's canonical review-input fingerprint. That fingerprint covers reviewer and authority, trigger and addressed condition, stage/scope/owner, governing revisions, candidate and integration base, effective effect, required-oracle/check and clearing-evidence state, and coordination/write/readback state. The result stays quiet only while the digest and every readable input remain unchanged; any change reopens the affected evaluation. Reliable read-only execution may update only bounded runtime-local lock, cursor, and hold state, never provider or project state. Provider writes with missing or unknown coordination/readback state enter reconciliation-only handling and are never retried blindly; an unauthorized missing write is held for the required authorization or human update rather than treated as executable.
