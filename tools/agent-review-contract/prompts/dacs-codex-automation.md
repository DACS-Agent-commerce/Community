...
With `READ_ONLY` or `DRAFT_ONLY`, including a configured submit effect downgraded to effective read-only, produce the authorized result once, then persist a runtime-local hold record keyed by reviewer, candidate, scope, base, effect, governing contract/policy revision, named clearing-evidence state, and addressed request/condition state. Carry that record across scheduled runs; while all keys remain unchanged classify `HOLD`, do not reassess it, and stay quiet. Arrival of any named clearing evidence, including a deciding-oracle result, or any newly addressed distinct condition takes priority and reopens only the affected evaluation; also reopen for any other key change, withdrawal, or supersession.
