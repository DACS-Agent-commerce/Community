...
If dirty against live `next`, use `COMMENT`, require refresh plus a new exact-head pass, and never approve from candidate-local tests alone. If stacked on an unmerged parent, `COMMENT` and hold until parent merge and child refresh; never assess an imagined merge. If a deciding oracle did not run, use bounded `HOLD` or `COMMENT`, name the missing oracle and exact next command/evidence, and let later same-pin evidence close only that hold. If an already-valid disposition exists on the same pin and scope, current base, reconciled effects, no open hold, and no new addressed request opens a distinct condition, report its ID/state without rerunning or rewriting.
