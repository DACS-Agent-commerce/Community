...
class PromptContractTest(unittest.TestCase):
    ...
    def test_prompts_separate_assessment_admission_and_publication_state(self):
        for prompt in PROMPTS:
            ...
            with self.subTest(prompt=prompt.name):
                ...
                self.assertIn("assessed stage", text)
                ...
                self.assertIn("Exclude trigger, owner, publication, coordination-write, and readback results", text)
                self.assertIn("current trigger and owner", text)
    ...
    def test_docs_bind_holds_to_complete_fingerprint(self):
        ...
        self.assertIn("assessed stage", readme)
        self.assertIn("Trigger, owner, publication, coordination-write, and readback results are excluded", readme)
        self.assertIn("admission fingerprint adds the current trigger and owner", readme)
